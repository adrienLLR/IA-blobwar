...
impl<'a> Configuration<'a> {
    ...
    /// Return the difference of blobs between the first player and the second player.
    /// It means that the first player want to maximize this value while the second
    /// player wants to minimize it. This is a simplification of MinMax algorithm.
    ...
    pub fn battle<T: Strategy, U: Strategy>(&mut self, mut player_one: T, mut player_two: U) -> i8 {
    ...
    /// Iterate on all possible jumps excluding the ones who doesnt win any blob
    fn jumps_valuable<'b>(&'b self) -> impl 'b + Iterator<Item = Movement> {
        self.blobs[self.current_player as usize]
            .positions()
            .flat_map(move |start| {
                // look at all distance 2 neighbours
                self.board.individual_neighbours[1][start as usize]
                    ...
                    .filter(move |&end| self.free_position_at(*end))
                    .map(move |end| Movement::Jump(start, *end))
                    .filter(move |mvt| self.value() != self.play(mvt).value())
            })
    }
    ...
    /// Iterate on all possible duplications for given player in random order.
    fn duplicates_random<'b>(&'b self) -> impl 'b + Iterator<Item = Movement> {
        let mut duplicates = self.empty_cells()
            .filter(move |&p| {
                !self.blobs[self.current_player as usize]
                    .intersection_with(self.board.neighbours[p as usize])
                    .is_empty()
        ...
        duplicates.shuffle(&mut rand::thread_rng());
        duplicates.into_iter()
    }

    /// Iterate on all possible sorted duplications for given player.
    fn duplicates_sorted<'b>(&'b self) -> impl 'b + Iterator<Item = Movement> {
        let mut duplicates = self.empty_cells()
        .filter(move |&p| {
            !self.blobs[self.current_player as usize]
                .intersection_with(self.board.neighbours[p as usize])
        ...
        if !self.current_player {duplicates.sort_by_key(|dup| -self.play(dup).value());}
        else {duplicates.sort_by_key(|dup| self.play(dup).value());}
        duplicates.into_iter()
    }
    ...
    /// Iterate on all possible moves, without taking the moves yielding 0 as a value.
    pub fn movements_valuable<'b>(&'b self) -> impl 'b + Iterator<Item = Movement> {
        self.duplicates().chain(self.jumps_valuable())
    }

    /// Iterate on all possible moves sorted.
    pub fn movements_sorted<'b>(&'b self) -> impl 'b + Iterator<Item = Movement> {
        self.duplicates_sorted().chain(self.jumps())
    }

    /// Iterate on all possible moves sorted.
    pub fn movements_random<'b>(&'b self) -> impl 'b + Iterator<Item = Movement> {
        self.duplicates_random().chain(self.jumps())
    }

    /// Take the element from an iterator and put it upfront. Doesnt check if m was in the iterator
    pub fn put_element_infront_iterator(iter: impl Iterator<Item = Movement>, m :Movement) -> impl Iterator<Item = Movement> {
        let one_elem_iter = std::iter::once(m);
        let new_iter = iter.filter(move |&x| x != m);
        one_elem_iter.chain(new_iter)
    }
